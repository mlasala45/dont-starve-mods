...
local function ShakeAllCameras(inst, mode, duration, speed, scale, maxDist)
    GetPlayer().components.playercontroller:ShakeCamera(inst, mode, duration, speed, scale, maxDist)
end
...
local function onexplode(inst)
    ...
    ShakeAllCameras(inst, "FULL", shakeduration, shakespeed, shakescale, shakemaxdist)
...
local function dostrike(inst)
    ...
    inst:DoTaskInTime(667 + FRAMES, inst.Remove)
end
