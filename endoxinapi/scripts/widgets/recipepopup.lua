...
local CyclingIngredientUI = require "widgets/cyclingingredientui"
...
function RecipePopup:Refresh()
	...
	local xoffset = center

	if num > 1 then
		xoffset = xoffset - (w/2 + div)*(num-1)
	end
	...
	for k,v in pairs(recipe.ingredients) do
		...
		if v.ingtype == "SPECIAL" then
			...
			has, num_found = amt >= v.amount, amt
		elseif v.ingtype == "GENERIC" or not v.ingtype then
			has, num_found = owner.components.inventory:Has(v.type, RoundUp(v.amount * owner.components.builder.ingredientmod))
		end
		...
		if v.ingtype == "VARIABLE" then
			ing = self.contents:AddChild(CyclingIngredientUI(v.ingredients, owner))
		else
		...
		ing:SetPosition(Vector3(xoffset, 80, 0))
		xoffset = xoffset + (w + div)
		self.ing[k] = ing
	end
end
